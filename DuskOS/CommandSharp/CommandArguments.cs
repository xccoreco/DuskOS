...
namespace CommandSharp.Commands
{
    public sealed class CommandArguments
    {
        ...
        //Ease of use
        public string GetEitherOr(char shortValue, string longValue)
        {
            var isShort = ContainsSwitch(shortValue);
            var isLong = ContainsSwitch(longValue);

            string result;
            if (isLong && !isShort)
            {
                result = GetArgumentAfterSwitch(longValue);
            }
            else if (!isLong && isShort)
            {
                result = GetArgumentAfterSwitch(shortValue);
            }
            else
            {
                throw new Exception("Can't make switch");
            }
